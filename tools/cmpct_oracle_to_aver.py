...
def main(path):
    ...
    lines = [
        ...
        "    exposes [key, shortIdFor, decodedNonce, decodedShortIds, decodedPrefilled, rebuiltRoot]",
    ...
    lines += [
        ...
        '    ? "The Transactions a node would have held unconfirmed, under the short"',
        '      "ids this Block names them by. Standing in for a Mempool: these are"',
        '      "the very Transactions Core expected us to have, which is why it sent"',
        '      "their names rather than their bytes."',
        "    Map.fromList([" + ", ".join(
            f"({sid}, {octets(raw)})" for sid, raw in zip(d["shortids"], d["txs_raw"][1:])
        ) + "])",
        "",
        "verify held",
