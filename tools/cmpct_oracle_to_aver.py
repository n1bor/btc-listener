...
def main(path):
    ...
    lines = [
        ...
        "    exposes [key, shortIdFor, decodedNonce, decodedShortIds, decodedPrefilled, rebuiltRoot, wantedNothingHeld, rebuiltBlock]",
        "    depends [Bytes, Domain.CompactBlock]",
        "    effects []",
        "",
        "fn payload() -> List<Int>",
    ...
    lines += [
        ...
        "",
        "fn rebuiltBlock() -> String",
        '    ? "The whole Block, rebuilt from the compact one, as hex."',
        '      "The strongest thing this corpus says. Not that the Merkle Root"',
        '      "matches, not that the short ids were found -- that the bytes which"',
        '      "come out are the bytes Core would have sent had it sent the Block"',
        '      "whole. Every ordering, every length prefix and every Transaction"',
        '      "serialisation has to be right for this to hold, and any one of them"',
        '      "being wrong is a Block that looks fine until something hashes it."',
        "    hexOf(Domain.CompactBlock.decodeCompact(payload()))",
        "",
        "verify rebuiltBlock",
        f'    rebuiltBlock() => "{d["raw_block"]}"',
        "",
        "fn hexOf(decoded: Result<Compact, String>) -> String",
        '    ? "The rebuilt bytes, or the reason there are none."',
        "    match decoded",
        "        Result.Err(why) -> why",
        "        Result.Ok(compact) -> builtHex(Domain.CompactBlock.rebuilt(compact, held(), []))",
        "",
        "verify hexOf",
        '    hexOf(Result.Err("no")) => "no"',
        "",
        "fn builtHex(built: Result<Bytes, String>) -> String",
        '    ? "Hex, or the reason the rebuild refused."',
        "    match built",
        "        Result.Err(why) -> why",
        "        Result.Ok(bytes) -> Bytes.toHex(bytes)",
        "",
        "verify builtHex",
        '    builtHex(Result.Err("no")) => "no"',
        "    builtHex(Result.Ok(Bytes.fromList([1]))) => \"01\"",
    ]
    open(OUT, "w").write("\n".join(lines) + "\n")
    print(f"wrote {OUT}")
