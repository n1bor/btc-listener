...
HEADER = '''module WitnessCases%(n)d
    intent =
        ...
        "names in the row. script_tests.json passes its flags to VerifyScript"
        "as they stand, so these read Domain.Rules.underFlags -- unlike"
        "tx_valid.json, which names the flags to leave out."
        "Standing as this was generated: %(cases)d cases, %(agree)d agree with"
        "Core, %(disagree)d disagree, %(undecided)d this engine cannot answer. Of"
        "the disagreements, %(lax)d are a rule this engine does not implement and"
        "%(strict)d are this engine refusing what Core accepts -- which is the"
        "direction a defect shows up in, so any of those is a bug until shown"
        "otherwise."
...
fn case(rawHex: String, prevouts: List<Prevout>, rules: Rules) -> Outcome
    ? "One whole Transaction, its single Input run against the Output Core's"
      "harness credited it with, under the rules the row's flags ask for."
...
def rules_literal(flags):
    ...
    read as it stands.  NONE is Core's spelling of the empty set.
    """
    ...
    for part in range(total):
        ...
        for i, (raw, txid, pubkey, value, flags, core_ok, comment) in enumerate(chunk):
            ...
            body.append('    case("%s", [%s], %s) => %s' % (
                raw, prevout_literal(txid, 0, pubkey, value), rules_literal(flags), answer))
        header = HEADER % dict(n=part + 1, total=total, cases=report["cases"],
                               agree=report["agree"], disagree=report["disagree"],
                               undecided=report["undecided"], lax=report["lax"],
...
def probe(path):
    ...
    for raw, txid, pubkey, value, flags, core_ok, comment in collected():
        lines.append('        ("%s", [%s], %s),' % (raw, prevout_literal(txid, 0, pubkey, value), rules_literal(flags)))
    with open(path, "w") as f:
        f.write(PROBE % dict(parts=parts_source(lines, "Tuple<String, List<Prevout>, Rules>")))
