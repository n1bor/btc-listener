...
name is added to its `depends` in the same PR. Measured at pin `600b3551`:
**55 universal, 3 bounded, 0 open, 66 declined** (the bounded three are the
two `when`-guarded Segment laws and ScriptParse's error-string law, which
the leaf reaches through Block). Four recursions were reshaped
